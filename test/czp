...
#define RECALL_ARRAY_LL(FUNC, OBJ, MEMTYPE, ARR_MARKER, TYPE, APD, STATE, ARG, ...) CAT(MERGE_AND_RECALL_, BOOL(APD))(FUNC, OBJ, MEMTYPE, ARR_MARKER, TYPE, STATE, ARG, __VA_ARGS__)
...
#define ARRAY_ARGS(FUNC, OBJ, MEMTYPE, ARR_MARKER, TYPE, APD, STATE, ARG, ...)\
  MAKE_FROM_PARAM_LL((FUNC, OBJ, MEMTYPE, ARR_MARKER, TYPE), APD, STATE, CHECK_ARG(ARG), PARENTHESIS(ARG), CHECK_ARG(__EXPAND_1 ARG), ARG, __VA_ARGS__)
...
#define MAKE_FROM_PARAM_LL(PARAM_IN_PAREN, APD, STATE, ARG_ISPRESNT, ARG_ISPAREN, PAREN_NONULL, ARG, ...) \
  IF_TRUE_EOR_RCURSE_CLL(ARG_ISPRESNT)\
    (IF_ELSE(ARG_ISPAREN, CAT(ASSERT_ARG_, PAREN_NONULL) (SIGUNUSED, ARG, CAT(ARG_VARGS_, PAREN_NONULL)(DO_NOTHING_LL(0), __VA_ARGS__)))(ASSERT_ARG_0(SIGBOUND, ARG, __VA_ARGS__)) \
    ...
    ELSE_STOPRCURSE_NEXPD_CLL(NOT(ARG_ISPRESNT))(IF_ELSE(NOT(CHECK_ARG(__VA_ARGS__)), ALIAS____(INIT_MACRO_CONSTRUCT, __EXPAND_1 PARAM_IN_PAREN, __EXPAND_1 STATE))(ASSERT_ARG_0(SIGEMPTY, ","ARG, __VA_ARGS__)))

#define INIT_MACRO_CONSTRUCT(FUNC, OBJ, MEMTYPE, ARR_MARKER, TYPE,  ...)\
  FUNC(OBJ.self, "<list::object>"#OBJ, MEMTYPE, ARR_MARKER, (void *)((cast[]){__VA_ARGS__}));
