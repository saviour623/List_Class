...
#define ARRAY(FUNC, NAME, OBJ, ...)				\
  SWITCH(PARENTHESIS(OBJ))(LOGMSG(SIGOBJPAREN, OBJ))		\
    ...
    (__VA_OPT__/* ASSERT_OBJ_TYPE(OBJ) */				\
     (IF_ELSE(CHECK_ARG(FUNC), __EXPAND4(ARRAY_ARGS(FUNC, NAME, OBJ, TYPE(CHOOSE_ARG(__VA_ARGS__)), WRAP_IN_PAREN(__VA_ARGS__), CHOOSE_VARGS(__VA_ARGS__))))	\
...
#define ARRAY_ARGS(FUNC, NAME, OBJ, TYPEID, ARG, ...)			\
  IF_ELSE(LOGICAL_OR(TYPEID, NOT(CHECK_ARG(ARG)))(1),	DECLARE_CONTENT_ARRAY(NAME, ARG, _____FILTER_ID(__EXPAND_1(__EXPAND1 CAT(T, TYPEID)))) \
	  __EXPAND_N_LOOP_IF_ELSE(CHECK_ARG(ARG), CAT(ASSERT_ARG_, CHECK_ARG(__EXPAND_1 ARG)) (SIGUNUSED, ARG, CAT(ARG_VARGS_, CHECK_ARG(__EXPAND_1 ARG)) (COPY_TOCONSTRUCT(FUNC, NAME, OBJ, CAT(T, TYPEID), ARG), __VA_ARGS__)), FUNC, NAME, OBJ, __VA_ARGS__) \
...
#define COPY_TOCONSTRUCT(FUNC, NAME, OBJ, TYPE_INFO, ARG)		\
  UNLESS(CHECK_ARG(FUNC))(CONSTRUCT_ARRAY(FUNC, NAME, OBJ, CHOOSE_1 TYPE_INFO, CHOOSE_2_ARG TYPE_INFO, __EXPAND_1 ARG))
