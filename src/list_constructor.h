...
#define ARRAY(FUNC, OBJ, MEMTYPE, ...)				\
  IF_ELSE(PARENTHESIS(OBJ), LOGMSG(SIGOBJPAREN, OBJ))(			\
    ...
    (__VA_OPT__(IF_ELSE(CHECK_ARG(FUNC), __EXPAND4(ARRAY_ARGS(FUNC, OBJ, MEMTYPE, 1, __VA_ARGS__))) \
		((NO_FUNCTION_PASSED_TO_ARRAY)0)))))

/* __ARRAY_ARGS__ */
#define ARRAY_ARGS(FUNC, OBJ, MEMTYPE, TYPEID, ARG, ...)		\
  __EXPAND_N_LOOP_IF_ELSE(CHECK_ARG(ARG), CAT(ASSERT_ARG_, CHECK_ARG(__EXPAND_1 ARG))\
       (SIGUNUSED, ARG, CAT(ARG_VARGS_, CHECK_ARG(__EXPAND_1 ARG))\
	(CONSTRUCT_ARRAY(FUNC, OBJ, MEMTYPE, 0, 1, __EXPAND_1 ARG), __VA_ARGS__)), FUNC, OBJ, MEMTYPE, 1, __VA_ARGS__) \
