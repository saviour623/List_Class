...
#define RECALL_ARRAY(FUNC, OBJ, MEMTYPE, ARR_MARKER, TYPE, APD, STATE, ARG, ...)\
...
#define ARRAY(FUNC, OBJ, MEMTYPE, ARR_MARKER, TYPE, ...)			\
  IF_ELSE(PARENTHESIS(OBJ), LOGMSG(SIGOBJPAREN, OBJ))(			\
    ...
    (__VA_OPT__(IF_ELSE(CHECK_ARG(FUNC), __EXPAND256(ARRAY_ARGS(FUNC, OBJ, MEMTYPE, ARR_MARKER, TYPE, 0, (), __VA_ARGS__))) \
		((NO_FUNCTION_PASSED_TO_ARRAY)0)))))

#define DO_NOTHING(...)
/* __ARRAY_ARGS__ */
#define ARRAY_ARGS(FUNC, OBJ, MEMTYPE, ARR_MARKER, TYPE, APD, STATE, ARG, ...) \
  IF_ELSE(CHECK_ARG(ARG), IF_ELSE(PARENTHESIS(ARG), CAT(ASSERT_ARG_, CHECK_ARG(__EXPAND_1 ARG)) (SIGUNUSED, ARG, CAT(ARG_VARGS_, CHECK_ARG(__EXPAND_1 ARG)) (RECALL_ARRAY(FUNC, OBJ, MEMTYPE, ARR_MARKER, TYPE, APD, STATE, ARG, __VA_ARGS__), __VA_ARGS__))) (ASSERT_ARG_0(SIGBOUND, ARG, __VA_ARGS__))) \
    (IF_ELSE(NOT(CHECK_ARG(__VA_ARGS__)), __EXPAND_1 STATE/* TODO: create array here*/)(ASSERT_ARG_0(SIGEMPTY, ","ARG, __VA_ARGS__)))
//efine ARRAY_ARGS(FUNC, OBJ, MEMTYPE, ARR_MARKER, TYPE, APD, STATE, ARG, ...) \
...
#define CONTINUE_WHILE_BOUNDED(FUNC, OBJ, MEMTYPE, ARR_MARKER, TYPE, APD)
  IF_ELSE(PARENTHESIS, CHECK_ARG(__EXPAND_1
